...
export const Alert: React.FC<AlertProps> = ({
  ...
  return (
    <div className={`rounded-md border p-4 ${style.bg} ${style.border} ${className}`} role="alert">
      <div className="flex">
        {showIcon && <div className="flex-shrink-0">{style.icon}</div>}
        <div className={`${showIcon ? 'ml-3' : ''} flex-grow`}>
          <h3 className={`text-sm font-medium ${style.text}`}>{message}</h3>
          {description && <div className={`mt-2 text-sm ${style.text}`}>{description}</div>}
        </div>
        {onClose && (
